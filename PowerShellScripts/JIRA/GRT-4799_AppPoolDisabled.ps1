...
[bool]$RapidFailProtection = $false
                                    ...
                                    if ($state.State -eq "Started") {
                                        ...
                                        try {
                                            ...
                                            if ($site) {
                                                if ($site.Count -gt 1) {
                                                    $web = $site[0]
                                                    $ResponseURI = Get-WebURL "IIS:\Sites\$web" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty ResponseUri
                                                }
                                                else {
                                                    $ResponseURI = Get-WebURL "IIS:\Sites\$site" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty ResponseUri 
                                                }
